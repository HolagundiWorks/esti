...
function toBbmpInput(preInput, actuals) {
    const { plotWidthM, plotDepthM } = estimatePlotDims(preInput.siteAreaSqm, preInput.plotWidthM, preInput.plotDepthM);
    ...
    const netBuiltUp = actuals
        ...
        : undefined;
    const coverPct = actuals?.groundCoverPct ??
    ...
    return {
        ...
        basementMechanicalParking: actuals?.basementMechanicalParking ?? preInput.basementMechanicalParking,
        ...
        actualSetbacks: actuals?.actualFrontSetbackM != null ||
...
function severityFor(deviation, allowed, limitType) {
    if (deviation == null || deviation <= 0)
        return "none";
    const pct = allowed !== 0 ? Math.abs(deviation) / Math.abs(allowed) : Math.abs(deviation);
    if (limitType === "MAX" && pct >= 0.15)
        return "critical";
    if (limitType === "MIN" && pct >= 0.25)
        return "critical";
    if (pct >= 0.05)
        return "major";
    return "minor";
}
function auditParam(key, label, unit, allowed, actual, limitType, extra) {
 ...
 */
export function computePreConstructionPotential(preInput, catalog = DEFAULT_BBMP_RULE_CATALOG) {
 ...
 */
export function computePostConstructionAudit(preInput, actuals, catalog = DEFAULT_BBMP_RULE_CATALOG) {
    const envelope = computeBylawEnvelope(preInput, catalog);
    ...
    const actualFar = bbmp.actualFar ??
    ...
    const parameters = {
        ...
        builtupArea: auditParam("builtupArea", "Built-up area", "sq m", envelope.permissibleBuiltup, netBuiltUp, "MAX"),
        groundCoverage: auditParam("groundCoverage", "Ground coverage", "%", envelope.coverageAllowed, coverPct, "MAX"),
        frontSetback: auditParam("frontSetback", "Front setback", "m", envelope.setbacks.front.value, actuals.actualFrontSetbackM, "MIN", { governedBy: trace.setbacks.front.governedBy }),
        rearSetback: auditParam("rearSetback", "Rear setback", "m", envelope.setbacks.rear.value, actuals.actualRearSetbackM, "MIN", { governedBy: trace.setbacks.rear.governedBy }),
        leftSetback: auditParam("leftSetback", "Left setback", "m", envelope.setbacks.left.value, actuals.actualLeftSetbackM, "MIN", { governedBy: trace.setbacks.left.governedBy }),
        rightSetback: auditParam("rightSetback", "Right setback", "m", envelope.setbacks.right.value, actuals.actualRightSetbackM, "MIN", { governedBy: trace.setbacks.right.governedBy }),
        parking: auditParam("parking", "Parking (ECS)", "ECS", envelope.parking.total, actuals.providedParkingEcs, "MIN"),
    };
    if (actuals.hasBasement) {
        parameters.basement = {
            ...
            reason: bbmp.basementCompliant === false
    ...
    const overallStatus = evaluated.length === 0
        ? "incomplete"
        : hasViolations
            ? "non_compliant"
            : "compliant";
...
export function preConstructionFromResult(envelope, bbmp) {
    return {
        farAllowed: envelope.farAllowed,
        permissibleBuiltup: envelope.permissibleBuiltup,
        coverageAllowed: envelope.coverageAllowed,
