...
export const siteInstructionsRouter = router({
  ...
  create: manage.input(SiteInstructionCreate).mutation(async ({ ctx, input }) => {    const { ref } = await nextRef(ctx.db, "siteinstruction", "SI");
  ...
  generatePdf: manage
    ...
    .mutation(async ({ ctx, input }) => {      const [row] = await ctx.db.select().from(siteInstructions).where(eq(siteInstructions.id, input.id));
