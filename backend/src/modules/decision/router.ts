...
const decisionInput = z.object({
  ...
  revisionSource: z.enum(["CLIENT_DRIVEN", "INTERNAL_ERROR", "TECHNICAL_QUERY", "SCOPE_CHANGE"]).optional(),
  /** Program Formulation — the frozen program version this revision is measured against. */
  programVersionId: z.string().uuid().nullable().optional(),
    ...
    const row = await ctx.db.transaction(async (tx) => {
      const [created] = await tx
        ...
        .values({
          ...
          revisionSource: input.revisionSource ?? null,
