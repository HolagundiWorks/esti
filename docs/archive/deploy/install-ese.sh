#!/usr/bin/env bash
# ARCHIVED (2026-07-10): ESE subdomain installer — no `ese/` package in this monorepo.
# Kept for historical reference. Do not run on production without restoring the ESE service.
#
...
#
#   sudo bash docs/archive/deploy/install-ese.sh
#
...
cp "$SCRIPT_DIR/nginx-ese.conf" "$ESE_CONF"
