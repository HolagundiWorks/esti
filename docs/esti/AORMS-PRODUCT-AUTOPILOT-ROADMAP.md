...
| P9.4 | Intelligence — firm-record Q&A + EOMS input-pack review; precedent search; deliverable lineage | 🔄 **in progress** — `ask` + `eomsReview` + **`precedentSearch`** + **`deliverableLineage`** + **`calcPackages`** (D4 lineage: `esti_cons_calc_package` + UI); **EOMS API on main**; optional: surface `eoms.*` UI; capacity analytics still open |
