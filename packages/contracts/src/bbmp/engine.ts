...
import {
  checkBasementCompliance,
  computeParkingEcs,
  computeRblSetback,
  engineConstants,
  farForProjectType,
  lookupFarRuleResult,
  lookupHighriseSetback,
  lookupLowriseSetbacks,
  secondaryComplianceFlags,
  secondaryRequirements,
} from "./rules.js";
import type {
  BbmpComplianceFlags,
  BbmpComplianceInput,
  BbmpComplianceResult,
  BbmpCalculationTrace,
  SetbackGovernance,
  SetbackSideResult,
} from "./types.js";

type SideKey = "front" | "rear" | "left" | "right";

function sideDistanceM(
  side: BbmpComplianceInput["front"],
): number {
  if (side.distanceCentreToBoundaryM > 0) return side.distanceCentreToBoundaryM;
  if (side.rblFromCentreM != null && side.rblFromCentreM > 0) return side.rblFromCentreM;
  return 0;
}

function tableSetbacks(
...
function governingSetbackForSide(
  input: BbmpComplianceInput,
  side: SideKey,
  table: Record<SideKey, number>,
  notes: string[],
  catalog: BbmpRuleCatalog,
): SetbackSideResult {
  const s = input[side];
  ...
  const rblSetback = computeRblSetback(
    s.roadWidthM,
    s.roadClass,
    sideDistanceM(s),
    catalog,
  ...
  const governedBy: SetbackGovernance = rblSetback > tableValue ? "RBL" : "Bylaw";
  if (governedBy === "RBL") {
    notes.push(
      `${side}: RBL governs (${rblSetback.toFixed(2)} m > bylaw ${tableValue} m)`,
    );
  }
...
function buildCompliance(
  ...
  setbacks: BbmpComplianceResult["setbacks"],
  parkingTotal: number,
  basementCompliant: boolean | null,
): BbmpComplianceFlags {
  const isFarCompliant =
    actualFar == null ? null : actualFar <= farAllowed;

  const isCoverageCompliant =
    input.proposedGroundCoverPct == null
      ? null
      : input.proposedGroundCoverPct <= coverageAllowed;

  let isSetbackCompliant: boolean | null = null;
  ...
  const isParkingCompliant =
    input.providedParkingEcs == null ? null : input.providedParkingEcs >= parkingTotal;

  const isBasementCompliant =
    !input.hasBasement ? null : basementCompliant !== false;
 ...
 */
export function computeBbmpCompliance(
  input: BbmpComplianceInput,
  ...
  const farLookup = lookupFarRuleResult(
  ...
  if (farLookup.basis === "road") {
    notes.push(
      `FAR and ground cover limited by governing road width (${governingRoadWidthM} m) — site area alone would fall in a higher band`,
    );
  } else if (farLookup.basis === "site") {
    notes.push(
      `FAR and ground cover from site-area band; governing road (${governingRoadWidthM} m) meets or exceeds the required width for that band`,
    );
  }
  ...
  if (input.buildingHeightM <= constants.lowriseHeightM) {
    notes.push(
      `Building ≤ ${constants.lowriseHeightM} m: setbacks from plot depth/width (Table 4)`,
    );
  } else {
    notes.push(
      `Building > ${constants.lowriseHeightM} m: uniform setbacks (Table 5)`,
    );
  }
  ...
  const parking = computeParkingEcs(
    input.projectType,
    permissibleBuiltup,
    input.dwellingUnits,
    input.unitAreaSqm,
    input.commercialFloorAreaSqm,
    catalog,
  );

  const basement = checkBasementCompliance(
    input.hasBasement,
    input.basementHeightM,
    input.basementMechanicalParking,
    input.basementProjectionAboveGroundM,
    catalog,
  );
  ...
  let actualFar: number | null = null;
  if (input.totalFloorAreaSqm != null && input.totalFloorAreaSqm > 0) {
    ...
    if (actualFar > farAllowed) {
      notes.push(
        `Actual FAR ${actualFar} exceeds allowed ${farAllowed} (after exemptions)`,
      );
    }
  }

  const secondaryCompliance = secondaryComplianceFlags(
    input.siteAreaSqm,
    plinth,
  ...
  const calculationTrace: BbmpCalculationTrace = {
    far: {
    ...
    secondary: secondaryRequirements(
      input.siteAreaSqm,
      plinth,
      input.buildingHeightM,
      input.floorCount,
