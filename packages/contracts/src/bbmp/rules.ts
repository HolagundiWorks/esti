...
export type ParkingFormulaKey =
  | "RESIDENTIAL_STANDARD"
  | "RESIDENTIAL_LARGE_UNIT"
...
export function engineConstants(
...
export const BBMP_FAR_RULES: FarRuleRow[] = [
  // Zone A — doc examples
...
export const BBMP_LOWRISE_SETBACK_RULES: LowriseSetbackRow[] = [
...
export const BBMP_HIGHRISE_SETBACK_RULES: HighriseSetbackRow[] = [
...
export const BBMP_ROAD_MARGINS: RoadMarginRow[] = [
...
export const BBMP_PARKING_RULES: ParkingRuleRow[] = [
  {
...
export const BBMP_SOLAR_RULES: SolarRuleRow[] = [
...
export const BBMP_SECONDARY_RULES: SecondaryRuleRow[] = [
  {
...
export function farForProjectType(row: FarRuleRow, projectType: BbmpProjectType): number {
  switch (projectType) {
    case "COMMERCIAL":
...
export function formatSqmBand(min: number, max: number): string {
...
export function formatRoadBandM(min: number, max: number): string {
  if (!Number.isFinite(max) || max >= OPEN_BAND) {
...
export function lookupFarRuleResult(
  ...
  const exact = zoneRules.find(
    (r) =>
      inBand(siteAreaSqm, r.siteAreaMin, r.siteAreaMax) &&
      inBand(governingRoadWidthM, r.roadWidthMin, r.roadWidthMax),
  );
  if (exact) return { row: exact, basis: "exact" };

  const siteMatches = zoneRules.filter((r) =>
    inBand(siteAreaSqm, r.siteAreaMin, r.siteAreaMax),
  );
  const roadMatches = zoneRules.filter((r) =>
    inBand(governingRoadWidthM, r.roadWidthMin, r.roadWidthMax),
  );
  ...
  if (siteMatches.length && roadMatches.length) {
    ...
    const roadLimited =
      ...
      roadMatches.reduce((best, r) =>
        farForProjectType(r, "RESIDENTIAL") < farForProjectType(best, "RESIDENTIAL") ? r : best,
      );
...
export function lookupLowriseSetbacks(
  plotDepthM: number,
  plotWidthM: number,
  catalog: BbmpRuleCatalog = DEFAULT_BBMP_RULE_CATALOG,
): {
  ...
  const depthRow =
    rows.find((r) => inBand(plotDepthM, r.depthMin, r.depthMax)) ??
    rows[rows.length - 1]!;
  const widthRow =
    rows.find((r) => inBand(plotWidthM, r.widthMin, r.widthMax)) ??
    rows[rows.length - 1]!;
  return {
...
export function lookupHighriseSetback(
  heightM: number,
  catalog: BbmpRuleCatalog = DEFAULT_BBMP_RULE_CATALOG,
): number {
  const rows = catalog.highriseSetbacks;
  const row =
    rows.find((r) => heightM >= r.heightMin && heightM < r.heightMax) ??
    rows[rows.length - 1]!;
  return row.uniformSetback;
}

export function roadMarginM(
  roadClass: RoadClass,
  catalog: BbmpRuleCatalog = DEFAULT_BBMP_RULE_CATALOG,
): number {
...
export function computeRblSetback(
  roadWidthM: number,
  roadClass: RoadClass,
...
export function lookupParkingRule(
  projectType: BbmpProjectType,
  unitAreaSqm: number,
  catalog: BbmpRuleCatalog = DEFAULT_BBMP_RULE_CATALOG,
): ParkingRuleRow {
  ...
  const match =
    rules.find((r) => inBand(unitAreaSqm, r.unitAreaMin, r.unitAreaMax)) ??
    rules[rules.length - 1];
  if (match) return match;
  return (
    catalog.parkingRules.find((r) => r.formulaKey === "COMMERCIAL_FLOOR_AREA") ??
    BBMP_PARKING_RULES[2]!
  );
}

export function computeParkingEcs(
  projectType: BbmpProjectType,
  ...
  catalog: BbmpRuleCatalog = DEFAULT_BBMP_RULE_CATALOG,
): { requiredECS: number; visitorECS: number; total: number; formulaKey: ParkingFormulaKey } {
...
export function secondaryRequirements(
  siteAreaSqm: number,
  plinthAreaSqm: number,
  buildingHeightM: number,
  floorCount: number,
  catalog: BbmpRuleCatalog = DEFAULT_BBMP_RULE_CATALOG,
): Array<{ ruleKey: SecondaryRuleKey; required: boolean; description: string }> {
  const rules = catalog.secondaryRules.length
    ? catalog.secondaryRules
    : BBMP_SECONDARY_RULES;
