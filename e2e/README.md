...
| `tests/crud-lifecycle.spec.ts` | **Edit + delete round-trips** — a BOQ line's full lifecycle (create → edit qty 10→20 so the total *re-derives* to ₹2,000 → remove the line, total back to ₹0); and a CRM archive (create a client → search → Deactivate, status flips Active → Deactivated). |
