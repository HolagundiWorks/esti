...
## Where the mismatch actually lies
...
billing to delivery. The financial heart of the practice stays in a spreadsheet and an
accounting package, disconnected from the work.

### Client approvals are invisible
...
- A **fee proposal** follows COA logic, and an **invoice** is GST-correct by
  construction.
...
system already has the shape of an architecture practice, so your work fits without
distortion.
...
There is also a recurring cost to running the practice across a generic tool plus the
folders, spreadsheets, and accounting package that fill its gaps: the **integration
tax**. Every day, someone keeps these in sync by hand. The project status in the tool
drifts from the real status in the folder; the fee in the spreadsheet drifts from the
