...
## What architecture office management software is

It is software that models an architecture office natively. Not tasks on a board,
but the real objects an office produces and depends on:

- **Projects and phases** that follow how a commission actually moves, from brief
  to completion.
...
Because these are first-class concepts rather than relabelled cards, the office's
real work lives inside the system instead of around it.
...
query or scope change — and whether it was minor, major or critical, so the cost of
...
Yes. A small practice can start with projects, drawings, fees, invoices and client
portals, and grow into team assignments, attendance, payroll and ASPRF performance
scoring as the office expands. The same record scales from a few people to a
fifty-person firm.
