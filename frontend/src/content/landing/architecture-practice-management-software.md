...
The signs are familiar. A junior asks how the office does fee proposals and gets three different answers. A client decision from last year cannot be found because it was in a WhatsApp thread. Two teams use different specifications for the same material. None of these are crises, but together they are the slow erosion of a practice's standards.
...
## Standards that hold across the practice

### One way to run a project

Every project in AORMS carries the same structure: phases, a drawing register with revisions and a current version, approval state, a transmittal log, and measurement sheets. Because the structure is the same for every job, a new team member learns it once and a partner can review any project without relearning how that team chose to organise itself. The shape of the work becomes a practice standard rather than a personal habit.
...
Architecture practice management software is a system that helps a practice run consistently as it grows beyond what one partner can personally oversee. It holds the standards, knowledge, people and client relationships in one place so every team runs projects the same way and the practice's institutional memory survives staff turnover.

### How does it preserve institutional memory?

Decisions are recorded rather than remembered. Revisions are tagged by category and source, comments and critical notes attach to the relevant record, and an immutable audit log and activity timeline keep the full history. When someone leaves, the reasoning behind a project's choices stays with the practice rather than walking out the door.
...
Every project uses the same structure, so a new joiner learns one way of working and a partner can review any team's project without relearning it. The ASPRF performance score also recognises mentoring, collaboration and learning, which encourages the senior knowledge-sharing that keeps quality high in a growing practice.
