...
Every change to a project touches the contract, even when nobody names it. The
...
[revision intelligence](/architecture-revision-tracking); what matters here is the
*consequence*. Lumping all four together is how margin disappears, because a scope
change billed as if it were a technical query is simply free work. Separating them
is what lets a practice say, with a straight face, "this one is on us, and this one
is a variation."
...
That single moment reframes change from a favour into a choice with a price, agreed
by both sides at the point of decision. It is far easier to say "yes, and here is
what it costs" in the moment than to reconstruct it from memory at the next
invoice. Major and Critical changes require explicit sign-off, so the practice
never absorbs a significant variation by reflex.

## Protecting margin with evidence, not memory
...
[GST billing for architects](/gst-billing-software-architects-india).

## The whole-office view of change

Change managed one project at a time still hides its pattern. AORMS rolls change
risk up to the office dashboard, so a principal can see which projects are
absorbing heavy change, which clients drive the most variations, and where internal
errors cluster. That view is a commercial instrument: it tells you which client
relationships need a firmer hand on scope, and which internal processes are costing
you in silent rework.
...
### How does it protect a project's margin?

By moving the cost conversation to the moment of decision. When a client accepts a
Major or Critical change through the portal, the timeline, cost and scope
consequence is shown before they confirm, and significant changes require explicit
sign-off. The practice then bills variations from a tracked record rather than
reconstructing them from memory.

### What is the difference between a scope change and a client request?

A scope change widens what was contractually agreed and is usually a fee variation,
while a client-driven request is a change the client asked for that is billable and
negotiable. A technical query, by contrast, is the design resolving itself and is
generally the office's own cost. AORMS keeps these distinct so they are handled
commercially, not lumped together as free work.

### Does change management connect to billing?

Yes. Client-driven and scope changes become billable items against COA Scale of
Charges fee proposals, invoiced with correct GST (CGST/SGST or IGST), TDS under
194J, the right SAC code and FY-sequential numbering. The trail from the change to
the invoice is one unbroken record.

### Can I see change across all projects at once?
