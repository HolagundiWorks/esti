...
**Architecture workflow management** makes the invisible process visible. It defines how work moves through the office � who has to approve, what gets sent out, what counts as done � and records every step so the trail is there when you need it.
...
Two tasks that look identical on a board can be billable design development and non-billable training. Tagging them properly means the office can see where effort really goes, and the same data feeds the ASPRF performance scoring � a fair, multi-dimensional 30-day composite � so workload and contribution are understood honestly rather than by gut feel.
...
For the wider picture, see [architecture firm management software](/architecture-firm-management-software) and, for change-heavy practices, [architecture change management software](/architecture-change-management-software). To see the workflows in action, [book a demo](/login).
...
### What does a transmittal record?

A transmittal logs exactly what was issued out � which drawing version, to which recipient, on which date � against the drawing register. When there is a dispute about what was sent or which revision a contractor built from, the transmittal log is the authoritative answer.

### Why tag tasks with both classification and work type?

Classification (billable, non-billable, training, collaboration, personal) captures the financial nature of the work, while work type (design communication, design development, technical production, construction support) captures the architectural nature. Together they show where effort really goes and feed the ASPRF performance scoring fairly.

### Can the audit trail be edited later?

No. The audit log is immutable, and an activity timeline records changes across the office. This means the record of who approved, issued or changed something is a true account that cannot be quietly rewritten afterwards.
