...
current state. AORMS holds internal approval workflows and client decisions on the
same record, so the entire chain of "who needs to say yes before this moves" is
one continuous, visible thing rather than two informal habits.
...
bottleneck. The bottleneck is already on screen: this client has held this revision
for three weeks; that internal sign-off has been pending since the drawing was
raised. Chasing becomes targeted instead of constant.

## Visibility across every project

Approvals are not trapped in a single project. They surface across the portfolio,
so the office can see every pending decision in one view rather than opening each
project in turn. For a busy practice this is the difference between discovering a
stalled approval because a deadline was missed, and seeing it the week it started
ageing.
...
An approval workflow only works if it is anchored to something concrete. In AORMS
the anchor is the issued drawing: a sheet goes out to a recipient through a
transmittal, and that issue is what starts the approval ageing. Because the
drawing register already holds current versions, superseded history and the
transmittal log (covered on the
[drawing revision tracking](/drawing-revision-tracking-software) page), an approval
is never an abstract checkbox. It is "this recipient owes a decision on this exact
revision, issued on this date." When they decide, the state updates on the same
record.
...
Charges, to GST-correct invoicing, to permits tracking, and to the bylaw
compliance checks a project has to clear (the free checker is at
[/compliance-check](/compliance-check)). Because AORMS is one system — self-hosted,
...
### How do clients give approvals?

Clients submit decisions through their own read-only portal, seeing only what has
been shared with them. Their confirmations come straight back onto the record as
tracked events, so there is no re-keying an email and no ambiguity about whether a
client actually agreed.

### How do I know which approvals are stuck?
...
### Can I see approvals across all projects at once?

Yes. Approvals surface across the portfolio rather than being trapped in a single
project, and the office dashboard and Action Center bring pending decisions
together with the rest of the office's live state, so a stalled approval is seen
the week it starts ageing.
