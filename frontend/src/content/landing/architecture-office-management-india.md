...
AORMS, powered by ESTI, is that system: a cloud-hosted Architecture Office Resource Management System where costing, projects, teams, clients, billing and site work share one source of truth.
...
The in-browser estimation module handles drawing takeoff and feeds quantities back into the system, so the numbers behind a BOQ come from the drawings rather than being re-typed.
...
Read-only client, consultant and contractor portals let outsiders see exactly what they should — drawings, status, documents — and nothing more. A dashboard of KPIs with an Action Center surfaces what needs attention, while an immutable audit log and activity timeline keep the whole office accountable.
...
It covers the commercial and operational sides that Indian offices live with daily — rate-book costing, tenders, contractor records, site work and India-native GST/TDS billing — alongside projects, phases, drawings, teams and client portals. AORMS keeps all of it in one cloud-hosted record instead of separate tools.
...
Yes. The knowledge bank carries rate books, a specification catalogue and bar bending schedules, and the in-browser estimation module feeds drawing takeoff quantities back in, so estimates and BOQs are grounded in Indian reference data.

### Can clients and consultants access the system?

They get read-only portals — separate client, consultant and contractor views — showing only the drawings, status and documents they are meant to see. The full office remains private, with role-based access controlling who inside the firm can do what.
