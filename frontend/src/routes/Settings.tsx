  ...
  return (
    <div>
      ...
      <Tile style={{ maxWidth: 520 }}>
        <Stack gap={5}>
          <h4>Display name</h4>
          <TextInput id="pf-name" labelText="Full name" value={name} onChange={(e) => setName(e.target.value)} />
          <Button
      ...
      <Tile style={{ maxWidth: 520, marginTop: 16 }}>
        <Stack gap={5}>
          ...
          <TextInput id="pf-cur" labelText="Current password" type="password" value={pw.current} onChange={(e) => setPw((p) => ({ ...p, current: e.target.value }))} />
          ...
          <Button
            disabled={
              !pw.current || pw.next.length < 8 || pw.next !== pw.confirm || changePassword.isPending
            }
            onClick={() => changePassword.mutate({ currentPassword: pw.current, newPassword: pw.next })}
          >
            {changePassword.isPending ? "Saving…" : "Change password"}
          </Button>
          {changePassword.error && (
            <InlineNotification kind="error" title="Could not change" subtitle={changePassword.error.message} hideCloseButton lowContrast />
          )}
        </Stack>
      </Tile>
    </div>
