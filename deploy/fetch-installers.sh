...
# 2. Download the three installers to a staging dir.
...
for f in "$LITE" "$CORE" "$ENT"; do
  [[ -f "$STAGE/$f" ]] || error "Release ${TAG} is missing ${f} — re-run the desktop-installer workflow."
done
info "Downloaded: ${LITE}, ${CORE}, ${ENT}"
...
set_env_kv VITE_CORE_DOWNLOAD_URL "/downloads/$CORE"
set_env_kv VITE_ENTERPRISE_DOWNLOAD_URL "/downloads/$ENT"
...
cp "$STAGE/$LITE" "$STAGE/$CORE" "$STAGE/$ENT" "$DIST.new/downloads/"
...
info "Done — https://<your-domain>/download now serves Lite / Core / Enterprise."
